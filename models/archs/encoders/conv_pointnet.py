...
class ConvPointnet(nn.Module):
    ...
    def __init__(self, c_dim=512, dim=3, hidden_dim=128, scatter_type='max', 
        ...
        self.fc_pos = nn.Linear(dim, 2*hidden_dim)  # dim=3, 2*hidden_dim=256
        ...
        self.plane_type: List[str] = plane_type
    ...
    def generate_plane_features(self, p: torch.Tensor, c: torch.Tensor, plane='xz') -> torch.Tensor:
    ...
    def forward(self, p, query):
        c = self.get_point_cloud_features(p.clone())

        plane_feat_sum = 0
        for plane in self.plane_type:
            plane_features = self.generate_plane_features(p, c, plane) # shape: batch, latent size, resolution, resolution (e.g. 16, 256, 64, 64)
            plane_feat_sum += self.sample_plane_feature(query, plane_features, plane)
    ...
    def get_point_cloud_features(self, p: torch.Tensor) -> torch.Tensor:
        batch_size, T, D = p.size() # T = 1024, D = 3
        ...
        net = self.fc_pos(p)    # p: (B, 1024, 3), net: (B, 1024, 256)")
    ...
    def get_plane_features(self, p: torch.Tensor) -> torch.Tensor:
        c = self.get_point_cloud_features(p)    # (B, 1024, 256)

        all_plane_features: List[torch.Tensor] = []
        for plane in self.plane_type:
            plane_features = self.generate_plane_features(p, c, plane) # shape: batch, latent size, resolution, resolution (e.g. 16, 256, 64, 64)
            all_plane_features.append(plane_features)

        return torch.cat(all_plane_features, dim=1)
